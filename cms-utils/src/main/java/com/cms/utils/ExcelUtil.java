...
import com.cms.utils.exception.ExcelException;
...
public class ExcelUtil {
	...
	public static <T> void listToExcel(List<T> list,
		...
		try {
			OutputStream out = response.getOutputStream();
			listToExcel(list, fieldMap, collectionName, title, content, sheetName, sheetSize, out);
		} catch (Exception e) {
			e.printStackTrace();
			if (e instanceof ExcelException) {// 如果是ExcelException，则直接抛出
				throw (ExcelException) e;
	...
	public static <T> void listToExcel(List<T> list,
		...
		//
		listToExcel(list, fieldMap, collectionName, title, content, sheetName, 65535, response, "");
	}
	 ...
	 *             异常
	 */
	public static <T> void leadToExcelQuestionBankTemplet(
			LinkedHashMap<String, String> fieldMap, String sheetName,
			HttpServletResponse response) throws ExcelException {
		// 设置默认文件名为当前时间：年月日时分秒
		String fileName = new SimpleDateFormat("yyyyMMddhhmmss").format(new Date()).toString();

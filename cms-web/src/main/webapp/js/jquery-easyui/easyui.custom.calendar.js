 ...
 * 二次開發 ____′↘夏悸
 * http://bbs.btboys.com Easyui中文社區
 */
(function ($) {
	...
	var monthName = new Array("正月", "二月", "三月", "四月", "五月", "六月", "七月", "八月", "九月", "十月", "冬月", "臘月");
	...
	function Calendar(y, m) {
		...
		for (var i = 0; i < this.length; i++) {
			...
			//依節氣調整二月分的年柱, 以立春為界
			/*
			//PM提出線上2月3日，初一不是庚寅年，應該是辛卯年。
			by yuji
			
			這裡firstNode是指農曆每月的節氣所在的日期，用這個標誌判斷
			農曆每月起始日天干地支是錯誤的，應當用每月的農曆初一所在日確定
			下月的天干地支。農曆每月初一都要重新計算一下天干地支。
			 */
	...
	function show(target) {
		...
		if (opts.year > 1874 && opts.year < 1909)
			yDisplay = '光緒' + (((opts.year - 1874) == 1) ? '元' : opts.year - 1874);
		if (opts.year > 1908 && opts.year < 1912)
			yDisplay = '宣統' + (((opts.year - 1908) == 1) ? '元' : opts.year - 1908);
		
		if (opts.year > 1911)
			yDisplay = '民國' + (((opts.year - 1911) == 1) ? '元' : opts.year - 1911);
//			yDisplay = '建國' + (((opts.year - 1949) == 1) ? '元' : opts.year - 1949);
		
		$(target).find('.calendar-title span').html(opts.months[opts.month - 1] + ' ' + opts.year + ' ' + yDisplay + '年 農曆 ' + cyclical(opts.year - 1900 + 36) + '年 【' + Animals[(opts.year - 4) % 12] + '年】');
				...
				day.hover(function (e) {
					...
					if (inf) {
						var ct = '<font color="#ffffff" style="font-size:9pt;">' + inf.sYear
							 ...
							 + '<br><font color="violet">農曆 ' + monthName[inf.lMonth - 1] + cDay(inf.lDay, inf.lMonth,true)
							 + '</font><br><font color="yellow">' + inf.cYear + '年 ' + inf.cMonth + '月 ' + inf.cDay + '日</font></font>';
						detail.html(ct);
